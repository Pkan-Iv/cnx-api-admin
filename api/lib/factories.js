...
export function CreateQueryHandler (pool) {
  return (query, resolve, reject) => (dispatch) => {
    console.log( '=>', query.replace( /[\n\r\t]/g, ' ' ).replace(/\s\s+/g, ' '))

    pool.getConnection( (err, connection) => {
      if (err)
        return dispatch(reject( err.message ))
...
export function CreateResponseHandler (res) {
  return (content = {}, code = 200) => {
    return res.status( code ).send(JSON.stringify( content ))
  }
}

export function CreateRouterMiddleware (actions) {
  return ({ getState }) => (next) => (payload) => {
    const result = next( payload ),
          { type } = payload

    if (actions[type] !== undefined) {
      return actions[type]( getState(), payload )
    }
